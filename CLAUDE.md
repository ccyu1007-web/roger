...
### 快速啟動
當使用者說「打開逍遙投資系統」或類似指令，自動判斷環境並執行：
1. 檢查 `/Users/roger/Documents/AI機器人/stock_system/` 是否存在，不存在就 `cd /Users/roger/Documents/AI機器人 && git clone https://github.com/ccyu1007-web/roger.git stock_system && cd stock_system`
2. 檢查 `stocks.db` 是否存在（且大小 > 1MB），不存在或太小就從 iCloud 還原：`cp ~/Library/Mobile\ Documents/com~apple~CloudDocs/stock_backup/stocks_latest.db stocks.db`（**注意：已存在的 DB 不要覆蓋**）
3. 檢查套件是否安裝，未安裝就 `pip3 install -r requirements.txt`
4. 檢查 port 5000 是否有 **python** 在跑（`lsof -i :5000 | grep python`），沒有就 `python3 app.py &`（macOS Monterey+ AirPlay 會佔 5000，要確認是 python 不是 AirPlay）
5. `open http://localhost:5000/`
6. 如果是新電腦（沒有 launchd 排程），提醒使用者要不要重建排程
...
#### Render 環境（is_cloud）
- **不觸發任何爬蟲**，純讀 DB
- 群益在海外 IP 會被擋，gunicorn timeout 120 秒會超時
- 所有資料靠 APScheduler 排程更新
...
長期6% = 權重股利 / 6%
```
- EPS = 預估EPS，沒有就用 min(沈董EPS, 綜合EPS)
- 股利 = 預估股利，沒有就用 min(沈董股利, 綜合股利)
...
3. `get_stocks` API 的 ALTER TABLE 區加欄位定義
4. `get_stocks` API 的 SELECT 加欄位
5. `sync_annual` 接收端的欄位清單加欄位
6. `_push_annual_to_render` 發送端的 SELECT 和 cols 加欄位
7. 前端移除獨立計算，改讀 API 回傳值

---

## 建立投資系統的開發流程（建新系統時參考）

以逍遙投資系統的經驗為基礎，記錄爬蟲、後端、前端的開發程序和踩坑教訓。
