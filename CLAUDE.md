...
- **EPS/股利取用順序統一**：使用者手動設定(vmEps) > Q1~Q4加總（手動填入或系統估算） > min(沈董EPS, 綜合EPS)。沈董EPS已改用本業推估法（保留季節性），綜合EPS含50%沈董+50%加權歷史。min()在成長股仍保留50%成長（透過blend），在極端季節性股票提供安全帽。股利跟隨EPS來源。預估等級和評價門檻都用同一組EPS，不可各自定義
