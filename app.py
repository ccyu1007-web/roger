...
def _calc_derived_fields(r, global_settings=None, user_params=None, qf_data=None):
    ...
    # (4) 用戶覆蓋（fwdNeffGrowth 優先，fallback 到 neffGrowth）
    if user_params:
        try:
            _ug = user_params.get('fwdNeffGrowth')
            if not (_ug is not None and str(_ug).strip()):
                _ug = user_params.get('neffGrowth')
...
def sync_annual():
    ...
    for r in rows:
        ...
        for extra in ['eps_ytd', 'eps_ytd_label', 'deepest_val_level', 'val_cheap_days',
                       ...
                       'blend_eps','blend_div','blend_pe','blend_yld','blend_grade',
                       'eps_4q_sum','trailing_div','trailing_pe','trailing_yld','trailing_grade',
                       'contract_1','contract_1q','contract_2','contract_2q','contract_3','contract_3q','contract_chg',
                       'payout_1','payout_2','payout_3','payout_4','payout_5','payout_6',
                       'val_aa','val_a1','val_a2','val_a','val_lt6',
