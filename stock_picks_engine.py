...
def fmt_value(r):
    ...
    ve = r.get('val_eps_used')
    vs = r.get('val_source', '')
    eps_tag = f'{ve:.1f}({vs})' if ve else '—'
    return f"| {c} | {n}{_liq(r)} | {r.get('fin_grade_1', '')} | {vl} | {cl} | {_fmt_m3(r)} | {_fmt_m12(r)} | {_fmt_rc(r)} | {_fmt_pe(r)} | {_fmt_yld(r)} | {eps_tag} | {ch} | {_fl(r)} |"
...
def run_full(dry_run=False):
    ...
    L.append("> EPS：沈董EPS（本業推估）> 綜合EPS（fallback）｜股利：跟隨EPS來源｜PE/殖利率：綜合")
    L.append("> 重倉：股價<=AA + 3M>=0 + 3M>=12M + 累積>0 + 防呆通過 + 近2年等級A以上")
    L.append("> 小買：股價<=A + 3M>=0 + 累積>0｜觀望：等級A以上 + 股價<=A + 營收未到位\n")
    VH = "| 代碼 | 名稱 | 等級 | 評價 | 股價 | 3M | 12M | 累積 | PE面 | 殖利率面 | EPS(來源) | 便宜天 | 防呆 |\n|------|------|------|------|------|-----|------|------|------|----------|-----------|--------|------|"
    VW_H = "| 代碼 | 名稱 | 等級 | 評價 | 股價 | 3M | 12M | 累積 | PE面 | 殖利率面 | EPS(來源) | 便宜天 | 等什麼 |\n|------|------|------|------|------|-----|------|------|------|----------|-----------|--------|--------|"
